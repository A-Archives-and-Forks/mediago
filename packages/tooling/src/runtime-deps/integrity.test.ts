...
const provisioner = readFileSync(
  path.join(repositoryRoot, "packages/tooling/src/runtime-deps/provisioner.ts"),
  "utf8",
);
const downloadCli = readFileSync(
  path.join(repositoryRoot, "packages/tooling/src/runtime-deps/download.ts"),
  "utf8",
);
const integritySource = readFileSync(
  path.join(repositoryRoot, "packages/tooling/src/runtime-deps/integrity.ts"),
  "utf8",
);
const depsVersions = JSON.parse(
  readFileSync(
    path.join(repositoryRoot, "packages/tooling/manifests/runtime-deps.json"),
    "utf8",
  ),
) as Record<
  string,
...
test("derives required checksum tools from the dependency layout", () => {
  expect(integritySource).toContain(
    'import { isDependencyToolName } from "./layout.ts";',
  );
