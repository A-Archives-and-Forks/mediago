...
import { RUNTIME_TOOLS } from "../runtime-deps/index.ts";
...
test.each(productionEntries)(
  ...
  ({ leaf, task }) => {
    ...
    const gateIndex = result.output.indexOf(
      "node packages/tooling/src/bootstrap/task-version-gate.ts",
    );
...
test("setup version gate runs without pnpm, tsx, package metadata, or node_modules", () => {
  ...
  const scriptsDirectory = path.join(
    fixture.directory,
    "packages/tooling/src/bootstrap",
  );
...
function createE2eBuildProfileFixture(): ReturnType<typeof createFixture> & {
  ...
  writeFileSync(
    ...
    [
      ...
      `const loaderUrl = ${JSON.stringify(new URL("../env/index.ts", import.meta.url).href)};`,
      "(async () => {",
...
function createCleanCloneProductionFixture(
  ...
  for (const filename of [
    "packages/tooling/src/bootstrap/task-version-gate.ts",
    "packages/tooling/src/env/index.ts",
    "packages/tooling/src/task/task-production-metadata.ts",
  ]) {
    const target = path.join(directory, filename);
    mkdirSync(path.dirname(target), { recursive: true });
    copyFileSync(path.join(repositoryRoot, filename), target);
  }
  writeFileSync(
  ...
  writeFileSync(
    ...
    [
      ...
      `const installedToolingModules = ${JSON.stringify(path.join(repositoryRoot, "packages/tooling/node_modules"))};`,
      ...
      'fs.symlinkSync(installedToolingModules, path.join(fixtureRoot, "packages/tooling/node_modules"), "dir");',
      "",
    ].join("\n"),
    "utf8",
  );
...
function createNodePrerequisiteFixture(
  ...
  if (target === "doctor") {
    return {
      ...
      helperName: "packages/tooling/src/bootstrap/task-doctor.ts",
      taskName: "doctor",
    };
  }
  ...
  return {
    ...
    helperName: "packages/tooling/src/bootstrap/task-version-gate.ts",
    taskName: "entry",
  };
}
