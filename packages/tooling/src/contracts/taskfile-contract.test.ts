...
const implementationGraph = {
  ...
  "internal:pack:extension": {
    ...
    leaves: ["pnpm pack:extension:raw"],
  },
  "internal:pack:electron": {
    deps: ["internal:deps:runtime", "internal:build:electron"],
  ...
  "internal:ci:desktop:validate-request": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/desktop-workflow.ts validate-request",
    ],
  },
  "internal:ci:desktop:verify-source": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/desktop-workflow.ts verify-source",
    ],
  },
  "internal:ci:desktop:artifact-prefix": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/desktop-workflow.ts artifact-prefix",
    ],
  },
  "internal:ci:desktop:apply-version": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/desktop-workflow.ts apply-version",
    ],
  },
  "internal:ci:desktop:create-test-stage": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/desktop-workflow.ts create-test-stage",
    ],
  },
  "internal:ci:desktop:upload-test-stage": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/desktop-workflow.ts upload-test-stage",
    ],
  },
  "internal:ci:desktop:release": {
    deps: ["internal:release:electron"],
    leaves: [],
  },
  "internal:ci:docker:validate-inputs": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/docker-workflow.ts validate-inputs",
    ],
  },
  "internal:ci:docker:resolve-parameters": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/docker-workflow.ts resolve-parameters",
    ],
  },
  "internal:ci:docker:verify-preview-private": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/docker-workflow.ts verify-preview-private",
    ],
  },
  "internal:ci:docker:detect-dockerhub": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/docker-workflow.ts detect-dockerhub",
    ],
  },
  "internal:ci:docker:resolve-targets": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/docker-workflow.ts resolve-targets",
    ],
  },
  "internal:ci:docker:write-summary": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/docker-workflow.ts write-summary",
    ],
  },
  "internal:ci:release:validate-request": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts validate-request",
    ],
  },
  "internal:ci:release:detect-release-state": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts detect-release-state",
    ],
  },
  "internal:ci:release:reserve-test-version": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts reserve-test-version",
    ],
  },
  "internal:ci:release:calculate-version": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts calculate-version",
    ],
  },
  "internal:ci:release:commit-version": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts commit-version",
    ],
  },
  "internal:ci:release:resolve-source": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts resolve-source",
    ],
  },
  "internal:ci:release:write-prepare-summary": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts write-prepare-summary",
    ],
  },
  "internal:ci:release:collect-electron-artifacts": {
    deps: [],
    leaves: [
      'node packages/tooling/src/electron-artifacts/cli.ts electron-artifacts release-files "$VERSION" "$UPDATER_CHANNEL"',
    ],
  },
  "internal:ci:release:download-test-stages": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts download-test-stages",
    ],
  },
  "internal:ci:release:publish-desktop": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts publish-desktop",
    ],
  },
  "internal:ci:release:record-test-docker": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts record-test-docker",
    ],
  },
  "internal:ci:release:write-desktop-summary": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts write-desktop-summary",
    ],
  },
  "internal:ci:release:tag-docker-release": {
    deps: [],
    leaves: [
      "node packages/tooling/src/github/release-workflow.ts tag-docker-release",
    ],
  },
} as const;
...
const prerequisiteGraph = {
  ...
  "internal:core:build:production": {
    deps: ["internal:deps:node"],
    leaves: ["pnpm core:build:production"],
  },
  "internal:test:e2e:build": {
    deps: ["internal:deps:node"],
...
describe("Task version gate and doctor", () => {
  ...
  it("contains exactly one typed minimum-version gate with environment-only inputs", () => {
    ...
    expect(gateCommands).toEqual([
      {
        kind: "cmd",
        text: "node packages/tooling/src/bootstrap/task-version-gate.ts",
      },
    ]);
    expect(
      gateCommands
  ...
  it("runs doctor through one static typed entrypoint", () => {
    expect(taskCommands("doctor")).toEqual([
      {
        kind: "cmd",
        text: "node packages/tooling/src/bootstrap/task-doctor.ts",
      },
    ]);
  ...
  it("keeps both bootstrap helpers independent of node_modules", () => {
    expect(
      bareBootstrapImports(
        "packages/tooling/src/bootstrap/task-version-gate.ts",
      ),
    ).toEqual([]);
    expect(
      bareBootstrapImports("packages/tooling/src/bootstrap/task-doctor.ts"),
    ).toEqual([]);
  });
...
describe("Task command leaves", () => {
  ...
  it("runs shared mutating prerequisites only once per invocation graph", () => {
    ...
    expect(task("internal:core:build:production").run).toBe("once");
  });
  ...
  it("gives only the environment-consuming shared E2E build its own test profile", () => {
    ...
    for (const property of ["vars", "env", "dotenv", "requires"]) {
      ...
      expect(task("internal:core:build:production")).not.toHaveProperty(
        property,
      );
    }
  });
  ...
  it("passes the canonical dependency root and current-platform leaf safely", () => {
    for (const name of runtimeConsumers) {
      ...
      expect(platform).toEqual({
        sh: "node packages/tooling/src/runtime-deps/print-platform-key.ts",
      });
...
describe("production variable requirements", () => {
  ...
  it("keeps public wrappers and reusable implementations metadata-free", () => {
    ...
    expect(task("internal:build:electron")).not.toHaveProperty("requires");
  });
    ...
    (name, expected) => {
      ...
      for (const variableName of expected) {
        expect(variables[variableName]).toEqual({
          sh: `node packages/tooling/src/task/task-production-metadata.ts ${variableName}`,
        });
        expect(
  ...
  it("loads the final production metadata value through the canonical profile loader", () => {
    const source = fs.readFileSync(
      path.join(
        repositoryRoot,
        "packages/tooling/src/task/task-production-metadata.ts",
      ),
      "utf8",
    );
    expect(source).toContain(
      'import { loadProfileEnv } from "../env/index.ts";',
    );
...
describe("Docker repository command contract", () => {
  ...
  it("uses the repository Node and pnpm versions with cacheable workspace manifests", () => {
    expect(dockerfileSource).toContain(
      "node:24.14.0-bookworm-slim AS node-builder",
    );
    expect(dockerfileSource).toContain(
      "corepack prepare pnpm@11.23.0 --activate",
    );
    expect(
      dockerfileSource.indexOf("COPY packages/tooling/package.json"),
    ).toBeLessThan(
      dockerfileSource.indexOf("RUN pnpm install --frozen-lockfile"),
    );
    expect(
      dockerfileSource.indexOf("COPY packages/ packages/"),
    ).toBeGreaterThan(
      dockerfileSource.indexOf("RUN pnpm install --frozen-lockfile"),
    );
    expect(dockerfileSource).toContain(
  ...
  it.each(Object.entries(normativeDocumentation))(
    ...
    (filename, requirements) => {
      ...
      if (requirements.installsTask) {
        ...
        expect(source).toContain(
          "packages/tooling/manifests/runtime-deps.json",
        );
