...
describe("build environment contract", () => {
  ...
  it("exposes only the three approved values to the UI bundle", () => {
    ...
    const source = readSource(configPaths.ui);

    expect(initializerText(config, "envPrefix")).toBe("[]");
    expect(source).toContain("mediaGoBuildMetadataPlugin({");
    expect(source).toContain("version: pkg.version");
    expect(source).toContain("target: process.env.APP_TARGET");
    expect(source).toContain("telemetryId: process.env.APP_TD_APPID");
    expect(source).not.toContain("VITE_UNAPPROVED_SECRET");
  });
  ...
  it("uses the shared profile loader in every Node-side environment consumer", () => {
    for (const filename of [
      ...
      const source = readSource(filename);
      expect(source).toContain('from "@mediago/tooling/env"');
      expect(source).toContain("loadProfileEnv(projectRoot)");
      expect(source).not.toContain("dotenvFlow.config");
    }
  });

  it("fails Electron packaging when runtime dependencies are unavailable", () => {
    const source = readSource(
      path.join(projectRoot, "packages/tooling/src/electron/build.ts"),
    );

    expect(source).toContain("process.env.MEDIAGO_DEPS_DIR?.trim()");
    expect(source).toContain(
      "Runtime dependency directory is missing or invalid",
    );
    expect(source).not.toContain("skipping deps copy");
  });
});
