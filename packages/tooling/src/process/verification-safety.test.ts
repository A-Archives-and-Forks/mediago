...
import type { DependencyManifestEntry } from "../runtime-deps/layout.ts";
import {
  BoundedRedactedLog,
  CleanupGate,
  attachSignalCleanup,
...
import { assertBBDownExecutableNames } from "../verification/verify-isolated-runtime-deps.ts";
...
test.runIf(process.platform !== "win32")(
  ...
  async () => {
    const helperUrl = new URL("./verification-safety.ts", import.meta.url).href;
    const fixture = `
      import { spawn } from "node:child_process";
